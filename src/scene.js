...
export class OceanScene{
 ...
 }else{const d=(L*.9+14)*this.zoom*(innerWidth<650?1.45:1.15);desired=new THREE.Vector3(s.x-Math.sin(angle)*d,h+d*(.11+this.lookPitch)+this.spec.bridgeY*.85,s.z+Math.cos(angle)*d);this.camera.position.lerp(desired,this.cameraSnap?1:1-Math.exp(-dt*5));this.camera.up.set(0,1,0);this.camera.lookAt(s.x,h+Math.max(2,this.spec.bridgeY*.6),s.z-L*.02);this.camera.fov=innerWidth<650?64:62;}
