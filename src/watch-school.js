...
export function watchHint(c){if(c.kind==='towline'||c.kind==='towing')return '예인선과 바지선 사이로 들어가지 마세요';if(c.kind==='net'||c.status==='fishing')return '양망 작업 · 부표와 어구 바깥으로 우회';if(c.kind==='workzone'||c.status==='restricted')return '수중 장비 작업 · 작업 구역 바깥으로 우회';if(['whale','shark'].includes(c.kind))return '감속하고 관찰 거리를 확보하세요';return '방위 변화와 접근 추세를 반복 관측하세요';}
