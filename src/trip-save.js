...
export function tripSnapshot(region,spec,state,cargo,mission,completed){return {version:1,region:region.id,ship:spec.id,position:{x:state.x,z:state.z,heading:state.heading},cargo,mission:mission?{originRegion:mission.originRegion,destinationRegion:mission.destinationRegion,originPort:mission.origin.id,destinationPort:mission.id,phase:mission.phase}:null,completed};}
export function restoreTrip(trip,region,spec){
 ...
 return {state:{...createVessel(),...p,anchored:true},cargo:{bays:[...c.bays],high:!!c.high,ballast:!!c.ballast},mission,completed};
}
