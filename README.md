...
## 1.17.4 — 수면을 타는 오토바이와 빠른 소리 메뉴

- 수상오토바이에 전용 수면 추종·활주 모델을 적용했습니다. 선체 주변 9개 지점에서 파도를 검사해 차체 상부가 파묻히지 않도록 하고, 하부와 제트 흡입구는 수면에 닿도록 유지합니다. 점프 이륙·착수 자세도 수정했습니다.
- 선수 양쪽에서 바깥·뒤로 퍼지는 저고도 물보라, 촘촘한 거품 자국, 착수 물보라. 공중에서는 새 수면 물보라·추진 거품을 만들지 않습니다.
- 오른쪽 위 ♪ 버튼: 짧은 고동, 긴 고동, 단음 3회·5회, 선박 종, VHF 모의교신을 바로 재생합니다. 다른 소리를 고르면 앞선 신호를 중단하고, 정지 버튼으로 교신 음성까지 중단할 수 있습니다.
- VHF는 수신 잡음과 브라우저 음성 합성으로 만든 게임용 모의교신입니다. 실제 무선 통신을 송수신하지 않습니다. 음성 합성 미지원 환경에서는 수신 잡음과 신호음을 재생합니다.
- 테스트 112개: 높은 파도·활주·착수 시 수면 유지, 물보라 방향과 공중 억제, 신호 스케줄·교체·정지 및 기존 항해 기능.

## 1.17.3 — 레이싱 좌우 버튼

- 파도 위 레이싱의 조타기를 양쪽 아래 좌·우 버튼으로 교체했습니다. 누르는 동안 회전하고 손을 떼면 중앙으로 돌아옵니다. 속도 레버는 오른쪽 버튼 위에 배치했습니다.
- A/D와 좌우 방향키도 누르는 동안 직접 조향합니다. 일반 항해의 조타기는 기존 방식입니다.
- 멀티 터치, 터치 취소, 포커스 이탈, 메뉴 열기와 홈 복귀 시 입력을 정리합니다. 레이싱 조향 응답은 선박 조타보다 빠르게 설정했습니다.
- 테스트 104개 및 모바일 390×844 화면에서 버튼 배치·조향·후진 이탈 확인.
...
- 코스 장애물·부표·경계, 선박·어망·예인줄, 육지·강변은 이동 경로에서 충돌을 검사하고 속도와 레버를 0으로 만듭니다. 충돌 페널티 대기 시간에도 관통을 차단합니다.
- 레이싱 충돌의 자동 부표 이동을 제거했습니다. 접촉 후에는 후진과 조타로 다시 주행할 수 있습니다. 일반 항해의 손상·수리 복구는 유지합니다.
- 충돌 규칙 변경으로 기존 레이싱 최고 기록은 새 규칙 기록과 분리됩니다. 어획 기록은 유지됩니다.
- 회귀 테스트 93개: 고속 관통, 반복 접촉, 후진 이탈, 다중 장애물, 지형 경계 및 기존 항해 기능.
...
### v1.16 — 작은 어선의 낚시, 수상오토바이 레이싱

시작 화면 또는 항해 메뉴에서 **낚시하며 쉬기** / **파도 위 레이싱**을 선택합니다. 선택한 놀이에 맞는 배로 바꾸며 현재 운송 의뢰와 화물을 초기화합니다. 바다 해역에서 시작할 수 있고, 지형과 정적 장애물로부터 충분히 떨어진 수역을 찾습니다.

- **낚시:** 찌 던지기 → 4–9초 입질 대기 → 4초 안에 챔질 → 버튼을 누르거나 길게 눌러 릴 감기. 장력이 가득 차기 전에 놓아야 합니다. 고등어·전갱이·참돔·농어 중 한 마리를 잡고 크기를 기록합니다. 놓아주기도 도감에 기록되며 최근 40마리를 저장합니다. 어종·크기·포획 확률은 게임용 설정으로 실제 어장이나 어업 규정의 재현이 아닙니다.
- **레이싱:** 3초 카운트다운 뒤 7개 부표를 순서대로 통과하는 실시간 타임 트라이얼입니다. 좌우 안내, 거리, 기록을 표시합니다. 후방 추적 카메라와 속력에 따른 시야각 확장, 기존 물보라 효과를 사용합니다. 접촉 시 3초 페널티를 받고 마지막 부표에서 재출발하는 아케이드 규칙입니다. 항해 모드의 손상·구조·수리 절차는 유지합니다. 개인 최고 기록을 브라우저에 저장합니다.
- **선박:** 작은 낚시 선실·낚싯대·어창을 가진 어선과 곡면 차체·핸들·부력 조끼 라이더를 가진 수상오토바이를 추가했습니다. 총 15척입니다.
- **화면:** 바다 청록과 포말색 계기, 넓은 수평선, 모드별 최소 조작부로 정리했습니다. 놀이 모드에서는 운송 안내와 대형 선박 계기를 숨깁니다. 모바일 390×844 및 데스크톱에서 확인했습니다.
- **검증:** 83개 테스트(입질·장력·어획, 체크포인트 순서·고속 통과·실제 선박 물리로 전체 코스 완주, 저장 검증 포함). 브라우저에서 어획·방류 기록과 실제 조타로 7개 부표 완주, 접촉 페널티·재출발, 모바일 가로 넘침 없음 및 실행 오류 없음을 확인했습니다.

### v1.16.1 — 바다가 먼저 보이는 화면

낚시 안내를 상단 창에서 하단의 작은 바로 옮겼습니다. 모바일 390×844에서 기본 바는 약 88px이며 릴 감기 중에만 작은 장력·진행 표시가 추가됩니다. 낚시 중 조타 패널은 기본적으로 숨기며, 상단 **계기** 버튼으로 필요할 때 펼칠 수 있습니다. **더보기**에서 어획 기록과 설명을 확인합니다.

일반 항해의 기본 조작부도 속도·방위·조타·레버·정박으로 줄였습니다. 레이더·롤링/피칭·날씨·배속은 **계기**에서 펼치고, 자유 항해에서는 목표 안내를 숨깁니다. 기본 화면의 타선 알림은 120m 이내 접근으로 제한하며 상세 계기를 열면 기존 관측 안내를 확인할 수 있습니다. 레이싱 목표도 하단으로 이동했습니다. 바다와 카메라는 유지하고 주변 암부 효과를 약하게 했습니다. 모바일 낚시·장력 표시·일반 조타·상세 계기 전환·레이싱 배치를 브라우저에서 확인했습니다.

### v1.17 — 수상오토바이 레이싱 코스

- 곡면 복합재 차체, 발판·핸들·시트, 헬멧과 바이저·부력 조끼·장갑·부츠를 갖춘 스포츠 라이더를 직접 모델링했습니다. 조타에 따라 라이더와 차체가 기울고 추적 카메라를 가까이 조정했습니다.
- 양쪽 부유 경계로 연결한 코스에 회피 장애물 3개와 점프대 2개를 배치했습니다. 전방으로 7m/s 이상 진입하면 속도에 따라 이륙하고 중력으로 착수하는 아케이드 점프입니다. 장애물은 감속·2초 페널티, 코스 이탈은 2초 페널티와 이전 부표 복귀입니다. 주변 선박 접촉은 기존 3초 규칙을 유지합니다.
- 항상 보이는 체크무늬 FINISH 아치를 만들고 마지막 결승 평면을 통과해야 완주하도록 했습니다. 부표 번호는 작게 줄이고 결승 아치를 가리는 원형 표시는 제거했습니다.
- 수상오토바이 항적이 양옆으로 더 넓게 퍼지고 물보라가 바깥쪽으로 분사됩니다. 경계 부표는 인스턴스로 묶어 렌더링합니다.
- 레이싱 휠은 왼쪽 아래, 레버는 오른쪽 아래, 기록·속도는 위쪽에 작게 표시해 중앙 모델을 가리지 않습니다. 시작 화면은 항해·낚시·레이싱 및 작은 학습 링크로 간소화했습니다.
- 변경된 코스의 기록은 이전 코스 기록과 분리하며 어획 기록은 유지합니다. 실제 수상레저 안전교육이나 경기 규정을 재현하는 모드는 아닙니다.
- 검증: 89개 테스트. 물리 기반 전체 코스 완주·점프·착수·장애물·코스 복귀·결승선 통과를 검사했습니다. 모바일 브라우저에서 두 차례 점프(관측 최고 높이 약 4.4m), 장애물 페널티, 전체 코스 완주와 화면 배치를 확인했습니다.

### v1.18.0 — 실제 접촉 판정과 해안 레이싱

- 타선의 레이더 주의 반경과 물리 선체를 분리했습니다. 표시 모델과 공유한 크기·방향의 선체 다각형을 연속 검사하며, 어망·예인줄·부선·작업 부표·표류목은 각각 보이는 위치에서 판정합니다. 주의 구역의 빈 물, 잠수한 생물, 정지 상태의 기존 겹침에는 새 사고를 부과하지 않습니다. 부두의 중복 원형 판정을 제거하고 방파제·타원형 모래톱을 실제 평면 크기에 맞췄습니다.
- 서해 인천, 남해 부산, 동해·묵호의 항구 인근에 코스를 배치합니다. 시작 메뉴와 지도에서 세 해안을 선택할 수 있습니다. 미니맵에는 현재 해역·내 위치·다음 부표·코스와 현재 해역의 모든 항구를 표시하며, 전체 항해도에는 24개 해역과 기존 항구 목록을 제공합니다. 해안 코스는 압축된 게임 지형의 연출이며 실측 수상레저 항로가 아닙니다.
- 롤·피치 각도와 게임 모델의 GM·복원 상태를 작은 한 줄로 표시합니다. 사고·복구·전복 안내를 하단 알림 영역에 모으고 사고 중 중복 토스트와 주변 경고를 숨겼습니다. 수리·구조 요청·복구 버튼은 유지했습니다.
- 검증: 125개 자동 테스트, 프로덕션 빌드, 공개 파일 검사. 근접 통과·실제 충돌·고속 타선 접근·그물·예인줄·정지 반복 판정과 세 해안 코스의 육지 회피 및 지도 범위를 검사했습니다. 데스크톱/390×844 모바일에서 코스 시작·지도·계기와 해역 전환을 확인했습니다.

### v1.19.0 — 항구에서 시작하는 화물 운송 학습

- 일반 항해의 상세 계기판(조타기·레이더·속도 레버)을 기본으로 펼칩니다. 계기 버튼으로 접을 수 있으며 새로 시작하거나 놀이 모드에서 돌아오면 기본 표시로 복구됩니다.
- 새 선적 미션은 출발 항구에 정박한 상태에서 명세 확인으로 시작합니다. 바다의 레저 선박에서 화물 미션을 선택하면 컨테이너선으로 승선합니다. 이미 진행 중인 운송은 이어 할 수 있습니다.
- 수량 확인 → 선적 → 고유 번호의 선적 B/L 발행 → 목적지 수하인 확인·B/L 회수·인도 승인 → 하역·운임 정산을 연결했습니다. 서류 진행 상태와 항차 손상 기록, 정산 내역을 저장합니다.
- 송하인·수하인·운송인·선적항·양하항을 명세에서 확인하고, 수량/수하인 불일치에는 설명을 제공합니다. 서류 판매가 아닌 운송 대가로 수익을 얻습니다. 게임 정산은 운임에서 항만비 8%를 빼고 무손상 항차에는 10% 보너스를 더합니다. 실제 법적 서류나 운임 계산을 재현하지 않습니다.
- 참고: [Maersk 운송약관](https://terms.maersk.com/carriage), [B/L 인도 안내](https://www.maersk.com/support/faqs/how-do-i-release-my-bill-of-lading-at-destination). B/L과 비유통 Sea Waybill의 차이를 게임 내 설명에 포함합니다.
- 검증: 132개 테스트. 서류 검증·발행·인도 조건·중복 정산 차단·손상 항차 보너스·저장 복원을 검사하고, 브라우저에서 컨테이너선 선적→자동 항해→입항→수하인 확인→하역을 진행했습니다. 390×844 모바일 기본 계기 표시를 확인했습니다.
